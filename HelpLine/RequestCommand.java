...
public class RequestCommand extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(@Nonnull GuildMessageReceivedEvent event) {
        ...
        if(msg[0].equalsIgnoreCase(Secret.Prefix + "request") && msg.length >= 3) {
            if(msg[1].equalsIgnoreCase("java") || msg[1].equalsIgnoreCase("cpp") || msg[1].equalsIgnoreCase("py")) {
                if(event.getChannel().getName().equalsIgnoreCase("cshelp")) {
                    if(msg[1].equalsIgnoreCase("java")) {
                        ...
                        Role role = event.getGuild().getRolesByName("JavaHelper", true).get(0);
                        tc.sendMessage(role.getAsMention() + " " + author + " needs help!\n" + send.trim()).queue();
                        ...
                        Role role = event.getGuild().getRolesByName("PythonHelper", true).get(0);
                        tc.sendMessage(role.getAsMention() + " " + author + " needs help!\n" + send.trim()).queue();
                        event.getChannel().sendMessage("``Message sent!``").queue();
                        ...
                        Role role = event.getGuild().getRolesByName("C++Helper", true).get(0);
                        tc.sendMessage(role.getAsMention() + " " + author + " needs help!\n" + send.trim()).queue();
                        event.getChannel().sendMessage("``Message sent!``").queue();
                    }
                }
